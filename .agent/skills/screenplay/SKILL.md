...
description: 확정된 시놉시스를 촬영 가능한 씬 단위 시나리오로 변환한다. 런타임 기반 밀도 공식, ACT-씬 타임코드 정합 규칙, 품질 게이트, 공통 규약(ID/승인/변경전파)을 포함.
---

# Screenplay Writing Skill v2.1
...
| 항목 | 형식 | 설명 |
|------|------|------|
| **입력** | synopsis JSON | `uid`, `status=approved/locked`, `info.runtime`, `acts[]`, `characters[]`, `keyScenes[]` |
| **출력 1** | scenes JSON (배열) | `uid`, `parent_uid`, `rev`, `status`, `number`, `scene_id`, `heading`, `action`, `dialogue`, `notes` |
...
- `status=approved` 또는 `locked`인 `scenes[]`만 줄콘티 스킬(`storyboard/SKILL.md`) 입력으로 전달한다.
- 변경 요청 시 JSON과 읽기용 본문을 함께 업데이트하고, `rev`를 증가시킨다.
- 씬 수정 시 해당 씬의 하위 자산(콘티·키비주얼·프리비즈·영상화)에 변경 전파 규칙(SKILL_INDEX §3)을 적용한다.

*Skill Version: 2.1.0*
*Created: 2026-02-07*
*Updated: 2026-02-20*
*Changelog: v2.0.0 — 런타임 기반 밀도 공식 추가, ACT-씬 정합 규칙 추가, 검증 함수 연동 명세*
*v2.1.0 — uid/parent_uid/rev/status 필드 추가(공통 규약 적용), 승인 게이트 반영, 변경 전파 규칙 연동*
