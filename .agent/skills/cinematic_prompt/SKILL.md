...
## 📊 프롬프트 스코어링 (Prompt Scoring) — v2.1 신설

### 목적
프롬프트의 실사 품질을 **정량적으로 평가**하여, 생성 전에 약점을 식별하고 보정한다.
키비주얼/영상화 단계의 QA에서 프롬프트 품질 기준선으로 사용한다.

### 채점 항목 (8개, 항목당 0~2점, 총 16점 만점)

| # | 항목 | 0점 (미달) | 1점 (기본) | 2점 (우수) |
|---|------|-----------|-----------|-----------|
| 1 | **스타일 선언** | Photorealistic 없음 | Photorealistic 있으나 촬영유형 미지정 | Photorealistic + 촬영유형 + 비율 + 해상도 |
| 2 | **카메라/렌즈** | 미지정 | 카메라만 있음 | 카메라 + 렌즈 + 초점거리 |
| 3 | **피사체 재질** | 추상 명사만 (armor, dress) | 재질 1개 명시 | 재질 + 색상 + 마모도/질감 |
| 4 | **조명** | 미지정 | 주광원만 있음 | 주광원 + 보조광 + 명암 패턴 + 렌즈 효과 |
| 5 | **색감/필름** | 미지정 | 색감만 있음 | 필름 에뮬레이션 + 색감 + 그레이딩 |
| 6 | **부정어** | 없음 | NOT 1~2개 | NOT 4개 이상 (illustration, painting, anime, 3D render) |
| 7 | **비유 배제** | 비유적 표현 포함 | 1~2개 남음 | 비유적 표현 완전 배제 |
| 8 | **맥락 구체성** | 범용 명사만 | 시대/장소 1개 명시 | 시대+장소+문화+건축양식 구체화 |

### 등급 판정

| 점수 | 등급 | 의미 | 조치 |
|------|------|------|------|
| 14~16 | **A** | 즉시 사용 가능 | 생성 진행 |
| 11~13 | **B** | 양호, 미세 보정 권장 | 부족 항목 1~2개 보정 후 생성 |
| 8~10 | **C** | 부분 보정 필요 | 부족 항목 보정 필수 |
| 0~7 | **D** | 재작성 필요 | 6단계 전체 재점검 |
...
1. 8개 항목 채점
2. 총점 계산 → 등급 판정
3. 1점 이하 항목 식별 → 해당 단계 가이드 참조하여 보강
4. B등급 이상이면 생성 진행
```
...
- `keyvisual/SKILL.md` — 키비주얼 생성 프로토콜 (앵커 잠금)
- `videoproduction/SKILL.md` — 영상화 프롬프트 (QA 루프와 연동)
- `previs/SKILL.md` — 프리비즈 (시각 일관성 검증)
...
*v2.0 → v2.1: 프롬프트 스코어링 시스템 추가(8항목/16점/4등급), 관련 스킬 연동 강화*
