...
> 필드명을 절대 변경하지 말 것.
> 📌 공통 규약: `SKILL_INDEX.md` §1(ID체계) §2(승인기계) 참조.

```json
{
  "uid": "<UUIDv7 — 생성 시 1회 발급>",
  "rev": "r1",
  "status": "draft",
  "title": "",
  "titleEn": "",
...
### 스키마 필드 규칙
- `uid`: UUIDv7. 생성 시 1회 발급, 수정 시 변하지 않음.
- `rev`: 리비전. 초안 `r1`, 수정 시 `r2`, `r3` 순차 증가.
- `status`: 승인 상태. `draft` → `review` → `approved` → `locked`. `rejected` 시 `draft`로 복귀.
...
## 다음 단계 Handoff
- `status`가 `approved` 또는 `locked`인 JSON만 시나리오 스킬 입력으로 넘긴다.
- 추가 피드백이 있으면 `assumptions`부터 우선 수정하고, `rev`를 증가시킨다.
- 시놉시스 수정 시 이미 작성된 하위 단계 산출물에 변경 영향도 전파 규칙(SKILL_INDEX §3)을 적용한다.

*Skill Version: 1.2.0*
*Updated: 2026-02-20*
*Changelog: v1.2.0 — uid/rev/status 필드 추가(공통 규약 적용), 승인 상태기계 반영, 변경 전파 규칙 연동*
